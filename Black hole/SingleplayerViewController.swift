...
import UIKit
// color 0 is red
class SingleplayerViewController: UIViewController, BlackHoleController {
    var tapNumber = 0
    ...
    func onTap(sec:section){
        if(color == 0 && tapNumber%2 != 0){
            return
        }
        if(color == 1 && tapNumber%2 != 1){
            return
        }
        ...
        if(color==1){
            let sections = board?.getFree()
            let number = arc4random_uniform(UInt32((sections?.count)!))
           // sections![Int(number)].fill(tapNumber/2 + 1, color: tapNumber%2)
            go((tapNumber/2) + 1, color: tapNumber%2)
            tapNumber++
            if(color==1){
                color = 0
            }
        ...
        if(sec.occupied==false){
            ...
            onTap(section(size: 0, row: 0, column: 0, x: 0, y: 0))
    ...
    func spacenumber(xCoordinate:Int, yCoordinate:Int) -> Int{
        let adjarr = board?.board[xCoordinate][yCoordinate].getAdjacent()
        var low = 0;
        for i in 0 ... (adjarr!.count-1){
   
        
            if(board?.board[adjarr![i][0]][adjarr![i][1]].color==1){
                low -= (board?.board[adjarr![i][0]][adjarr![i][1]].number)!
            }
            else if (board?.board[adjarr![i][0]][adjarr![i][1]].color==1 && board?.board[adjarr![i][0]][adjarr![i][1]].number != 0) {
                low += (board?.board[adjarr![i][0]][adjarr![i][1]].number)!
            }else{
                low -= 400
            }
    ...
    func go(number:Int, color:Int){
        var lowestnum = -100000000
        var space = section(size: 0, row: 0, column: 0, x: 0, y: 0)
        for x in (board?.board)!{
            for y in x{
                if(y.occupied==false){
                    if(spacenumber(y.row, yCoordinate: y.column)>lowestnum){
                        lowestnum = spacenumber(y.row, yCoordinate: y.column)
                        space = y
                    }
                }
            }
        }
        space.fill(number, color: color)
    }
